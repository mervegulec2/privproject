...
class FlowerPrototypeClient(fl.client.NumPyClient if fl is not None else object):  # type: ignore[misc]
    ...
    def fit(self, parameters, config):
        ...
        use_cw = False # No class-weighted distribution for FL
        class_weights = None

        progress = os.environ.get("PROGRESS", "0") != "0"
        lambda_p = float(os.environ.get("LD", os.environ.get("LAMBDA_P", "0.1")))
        train_local_proto(
            self.model, train_loader, global_protos, self.cfg, 
            lambda_p=lambda_p, progress=progress, cid=self.cid, 
            class_weights=class_weights, individual_proto_loss=True
        )
        ...
        if self.security_manager:
            result = self.security_manager.apply_defenses(local_protos, counts=local_counts)
            if isinstance(result, tuple) and len(result) == 2:
                local_protos, local_counts = result
            else:
                local_protos = result
...
def run_flower_experiment(
    *,
    ...
    train_backbone = True # Always train full model for FL
    train_head = True
...
def main():
    ...
    run_flower_experiment(
        ...
        test_ds=test_ds,
        split=split,
        metrics_csv_path=os.path.join(base, "simulation_results_fl.csv"),
        plot_path=os.path.join(base, "accuracy_curve_fl.png"),
    )
