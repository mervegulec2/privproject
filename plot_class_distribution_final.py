...
def plot_distribution_pro(seed=42, alpha=0.1, num_clients=10):
    ...
    # Client 0-9 to match terminal table
    clients = [f"Client {i}" for i in range(num_clients)]
    
    # Standard Tab10 palette
    colors = plt.cm.tab10(np.linspace(0, 1, 10))
    
    left = np.zeros(num_clients)
    for c in range(10):
        # Legend labels as Class 0, Class 1, etc.
        plt.barh(clients, distribution_norm[:, c], left=left, label=f"Class {c}", color=colors[c], edgecolor='white', linewidth=0.5)
    ...
    # Match table order (Client 0 at top)
    plt.gca().invert_yaxis()
    
    plt.tight_layout()
    
    save_path = f"outputs/metrics/class_dist_final.png"
